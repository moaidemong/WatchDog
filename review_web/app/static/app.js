...
function currentQuery() {
  return new URLSearchParams({
    ...
    review_status: document.getElementById("status-filter").value,
    review_label: document.getElementById("label-filter").value,
...
async function loadRows() {
  ...
  payload.items.forEach((item, index) => {
    ...
    const labelCell = tr.children[5];
    const statusCell = tr.children[6];
    const notesCell = tr.children[7];
