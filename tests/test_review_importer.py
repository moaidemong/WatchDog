...
from app.review.importer import LABEL_COLUMNS, ReviewLabelImporter
...
def test_review_label_importer_upserts_existing_event_rows(tmp_path: Path) -> None:
    artifacts_dir = tmp_path / "artifacts"
    event_dir = artifacts_dir / "event-1"
    event_dir.mkdir(parents=True)
    metadata_path = event_dir / "metadata.json"
    ...
    assert len(rows) == 1
    assert rows[0]["review_label"] == "failed_get_up_attempt"
    assert rows[0]["review_notes"] == "updated note"
